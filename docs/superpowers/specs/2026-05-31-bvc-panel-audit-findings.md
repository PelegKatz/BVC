...
| F2 | Detection | Select a tag-based `cxui-*` component not in DETECT_MAP (e.g. `cxui-form-field`, `cxui-avatar`) in isolated world | Resolve its catalog entry + Properties section | `isCxuiComponent` true (tag check) but `findCxuiEntry` returns **null** → renders "uncataloged cxui" empty state; `describeCxuiComponent` shows raw `cxui-form-field` | P1 | `catalog-cxui.ts` — DETECT_MAP only covers 6 components; no tag-derived name fallback |
