...
export function createBlockedEmptyState(props: BlockedStateProps): HTMLDivElement {
  ...
  wrap.className = 'bvc-empty-state';
  ...
  title.textContent = isChart ? `<${blockingTag}> is driven by chart config — not editable here` : `<${blockingTag}> isn't editable in BVC`;

  const body = document.createElement('div');
  body.className = 'bvc-empty-state-body';
  body.textContent = isChart
    ? "This element is rendered from the parent `<cxui-chart>`'s `[config]` input. BVC overrides on the DOM node would get overwritten on the next chart render. Copy a prompt that tells Claude exactly what to edit in source, or jump out to the surrounding layout container."
    : "Visualization components (cxui-graph) have config-driven rendering that doesn't map to BVC sections. Jump out to the surrounding layout container to adjust placement.";

  wrap.append(icon, title, body);

  const actions = document.createElement('div');
  actions.className = 'bvc-empty-state-actions';

  const prompt = buildChartConfigPrompt(props.blockingAncestor);
  if (prompt) {
    const copyBtn = document.createElement('button');
    copyBtn.type = 'button';
    copyBtn.className = 'bvc-empty-state-action bvc-empty-state-action-primary';
